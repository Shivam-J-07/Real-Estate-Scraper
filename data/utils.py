...
def get_headers(base_url):
    ...
    # Loop to ensure the user agent is for Mac OS Chrome
    while not ('mac' in user_agent.lower() and 'chrome' in user_agent.lower()):
        user_agent = UserAgent().random

    # Set the headers to mimic a browser request from Chrome on Mac
    headers = {
        ...
        'Sec-Ch-Ua-Platform': '"macOS"',  # Platform information
